...
class HumanPlayer: public AbstractPlayer
{
    public:
        ...
        void announce(int playerID, ComputerPlayer& casino)
        {
...
class BlackJackGame
{
    public:
        /***
         * helper method for allowing player to decide how many hands they want to play with (1/2/3)
         * called in BlackJackGame::play()
         ***/
        int setNumOfPlayerHands() {
            string input;
            while (input != "1" && input != "2" && input != "3")
            {
                cout << "How many hands would you like to have? (1/2/3):";
                cin >> input;
                if (input == "1" || input == "2" || input == "3")
                {
                    numOfPlayerHands = stoi(input);
                    for (int i = 0; i < numOfPlayerHands; i++)
                    {
                        m_players.push_back(HumanPlayer(m_deck));
                    }
                }
                else {
                    cout << "Incorrect value entered. The minimum number of hands allowed is 1 and the maximum allowed is 3. Please try again." << endl;
                }
            }
            return -1;
        }
        int play()
        {
            try {
                char answer;
                // let Player decide number of hands they want to play with
                BlackJackGame::setNumOfPlayerHands();
                ...
                for (int i = 0; i < numOfPlayerHands; i++)
                {
                    ...
                    deal<HumanPlayer>(m_deck, m_players.at(i));
                    deal<HumanPlayer>(m_deck, m_players.at(i));

                    // display each hand's first 2 cards
                    cout << "Player " << i+1 << ": ";
                    m_players.at(i).displayCards();

                    while (m_players.at(i).isDrawing())  // Player's total < 21, their turn to draw
                    {
                        ...
                        if (answer == 'y')  // Player decides to draw (again)
                        {
                            ...
                            cout << "Player " << i+1 << ": ";
                            m_players.at(i).displayCards();
                            if (m_players.at(i).isBusted() || m_players.at(i).getTotal() == 21)  // Player score >= 21, can no longer draw
                            {
                                break;
                            }
                        }
                ...
                {
                    /***
                     * compare individual Player hand to Casino's hand
                     * win/loss/push determined for each pair (Player 1 vs. Casino, Player 2 vs. Casino, Player 3 vs. Casino)
                     ***/
                    m_players.at(i).announce(i+1, m_casino);
                }
            ...
            if (m_casino.getTotal() == 0 && m_deck.getTotal() == 0)
            {
                return 0;
            }
            else
